...
public String cancelBooking(GeneralBean generalBean)
{
	...
	try {
		...
		ps=connection.prepareStatement("select * from ocsr_booking where BOOKING_ID= ?");
		...
		if(rs.next())
		{
			...
			psDelete=connection.prepareStatement("Delete from ocsr_booking where BOOKING_ID=?");
